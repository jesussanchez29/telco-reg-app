...
@st.cache
def cargar_modelo():
    with open('models/churn-model.pck', 'rb') as file:
        dv, modelo_regresion = pickle.load(file)  # Desempaquetar la tupla
    return dv, modelo_regresion
...
def preprocesar_datos(input_data):
    input_data['gender'] = (input_data['gender'] == 'Masculino').astype(int)
    input_data['partner'] = (input_data['partner'] == 'Sí').astype(int)
    input_data['dependents'] = (input_data['dependents'] == 'Sí').astype(int)
    input_data['phoneservice'] = (input_data['phoneservice'] == 'Sí').astype(int)
    input_data['multiplelines'] = input_data['multiplelines'].apply(lambda x: 1 if x == 'Sí' else (0 if x == 'No phone service' else -1))
    input_data['internetservice'] = input_data['internetservice'].apply(lambda x: 0 if x == 'No internet service' else (1 if x == 'DSL' else 2))
    input_data['onlinesecurity'] = input_data['onlinesecurity'].apply(lambda x: 1 if x == 'Sí' else (0 if x == 'No internet service' else -1))
    input_data['onlinebackup'] = input_data['onlinebackup'].apply(lambda x: 1 if x == 'Sí' else (0 if x == 'No internet service' else -1))
    input_data['deviceprotection'] = input_data['deviceprotection'].apply(lambda x: 1 if x == 'Sí' else (0 if x == 'No internet service' else -1))
    input_data['techsupport'] = input_data['techsupport'].apply(lambda x: 1 if x == 'Sí' else (0 if x == 'No internet service' else -1))
    input_data['streamingtv'] = input_data['streamingtv'].apply(lambda x: 1 if x == 'Sí' else (0 if x == 'No internet service' else -1))
    input_data['streamingmovies'] = input_data['streamingmovies'].apply(lambda x: 1 if x == 'Sí' else (0 if x == 'No internet service' else -1))
    input_data['contract'] = input_data['contract'].apply(lambda x: 0 if x == 'Mes a mes' else (1 if x == 'Un año' else 2))
    input_data['paperlessbilling'] = (input_data['paperlessbilling'] == 'Sí').astype(int)
    input_data['paymentmethod'] = input_data['paymentmethod'].apply(lambda x: 0 if x == 'Banco' else (1 if x == 'Cheque electrónico' else (2 if x == 'Transferencia bancaria' else 3)))
